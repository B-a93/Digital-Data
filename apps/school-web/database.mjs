...
export async function initializeDatabase() {
  ...
  try {
    ...
    ready = false;
    lastError = error instanceof Error ? error.message : 'Database initialization failed';
...
export async function databaseHealth() {
  if (!pool || !ready) return { ok: false, configured: Boolean(connectionString), error: lastError };
  try {
    await pool.query('SELECT 1');
    return { ok: true, configured: true };
  } catch (error) {
    ready = false;
    lastError = error instanceof Error ? error.message : 'Database health check failed';
