...
export function seed() {
  ...
  return {
    ...
    students: names.map((name, i) => ({
      id: `s${i + 1}`,
      admission: `DEMO-${String(i + 1).padStart(4, "0")}`,
      name,
      class: classes[i % 4],
    })),
    attendance: {},
    charges: names.map((_, i) => ({
      id: `c${i + 1}`,
      studentId: `s${i + 1}`,
      amount: 150000,
      label: "Term tuition",
...
export function cents(value) {
  ...
  if (!Number.isSafeInteger(n) || n <= 0 || n > 100000000)
    throw new Error("Amount must be between D0.01 and D1,000,000.");
  return n;
}
...
export function recordPayment(state, { studentId, amount, operation, date }) {
  ...
  if (previous) {
    if (
      ...
      throw new Error("Payment operation already used with different details.");
    return previous;
  }
  const payment = {
    id: operation,
    ...
    amount,
    date,
    reference: `DEMO-R${String(state.payments.length + 1).padStart(4, "0")}`,
  };
...
export function attendanceSummary(marks) {
  ...
  const eligible = values.filter((v) =>
    ["present", "late", "absent"].includes(v),
  );
  const attended = eligible.filter(
    (v) => v === "present" || v === "late",
  ).length;
  return {
    marked: values.filter((v) =>
      ["present", "late", "absent", "excused"].includes(v),
    ).length,
...
export function publishResults(state, className, subjectName = "General") {
  ...
  if (
    ...
    throw new Error(
      "Enter a valid mark from 0 to 100 for every student in this class.",
    );
  const version =
    state.published.filter(
      (p) =>
        p.class === className &&
        (p.subject || "General") === subjectName &&
