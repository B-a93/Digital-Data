...
function printStudentIdCards() {
  const students = activeStudents().filter(
    (student) => !studentClass || student.class === studentClass,
  );
  if (!students.length) {
    toast("No active students are available for this class.");
    return;
  }
  const popup = window.open("", "_blank");
  if (!popup) {
    toast("Allow pop-ups to open the printable ID cards.");
    return;
  }
  const cards = students
    .map(
      (student) =>
        `<article class="id-card"><div class="id-head"><span class="school-mark">${esc(state.settings.name.charAt(0).toUpperCase() || "S")}</span><div><strong>${esc(state.settings.name)}</strong><small>STUDENT IDENTIFICATION CARD</small></div></div><div class="id-body"><div class="photo">${esc(student.name.charAt(0).toUpperCase())}</div><div class="details"><h2>${esc(student.name)}</h2><p><span>Student number</span><strong>${esc(student.admission)}</strong></p><p><span>Class</span><strong>${esc(student.class)}</strong></p><p><span>Term</span><strong>${esc(state.settings.term)}</strong></p></div></div><div class="guardian"><span>Guardian</span><strong>${esc(student.guardianName || "Not recorded")}</strong><span>${esc(student.guardianPhone || "No phone number")}</span></div><footer>Generated by School Management Portal · Elegant Empire AI</footer></article>`,
    )
    .join("");
  popup.document.write(
    `<!doctype html><html><head><meta name="viewport" content="width=device-width,initial-scale=1"><title>${esc(studentClass || "All students")} ID Cards</title><style>@page{size:A4 portrait;margin:10mm}*{box-sizing:border-box}body{font:11px Arial;color:#17322b;margin:0}.sheet{display:grid;grid-template-columns:repeat(2,1fr);gap:8mm 7mm}.id-card{height:59mm;border:1px solid #aec7bd;border-radius:10px;overflow:hidden;break-inside:avoid;background:#fff;position:relative}.id-head{height:17mm;background:#146a56;color:white;display:flex;align-items:center;gap:10px;padding:8px 12px}.id-head strong{display:block;font-size:14px}.id-head small{display:block;font-size:7px;letter-spacing:1px;margin-top:3px}.school-mark{width:33px;height:33px;border-radius:50%;background:white;color:#146a56;display:grid;place-items:center;font-size:19px;font-weight:bold}.id-body{display:flex;gap:12px;padding:10px 12px 7px}.photo{width:48px;height:55px;background:#eaf4ef;border:1px solid #c6dacf;border-radius:7px;display:grid;place-items:center;color:#146a56;font-size:25px;font-weight:bold}.details{flex:1}.details h2{font-size:14px;margin:0 0 5px}.details p{display:flex;justify-content:space-between;gap:8px;margin:3px 0;font-size:9px}.details span,.guardian span{color:#60736d}.guardian{border-top:1px solid #e3ece8;margin:0 12px;padding:6px 0;display:flex;gap:6px;align-items:center;font-size:8px}.guardian strong{margin-right:auto}footer{position:absolute;bottom:0;left:0;right:0;background:#f2f6f3;padding:4px 12px;font-size:6px;color:#60736d}.print{margin:0 0 12px;padding:9px 14px;background:#146a56;color:white;border:0;border-radius:6px}@media(max-width:650px){.sheet{grid-template-columns:1fr}.id-card{height:62mm}}@media print{.print{display:none}}</style></head><body><button class="print" onclick="window.print()">Print or save as PDF</button><main class="sheet">${cards}</main></body></html>`,
  );
